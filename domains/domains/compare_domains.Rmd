...
####MEROPS analysis####
mer.key <- read.delim("../lib/merops_lib.families.tab", header=FALSE)
head(mer.key)
...
mer.files=data.frame(names=str_replace(list.files("MEROPS/"), ".blasttab", "")) %>%
  ...
  mutate(source=as.character(1:length(loc)), Genome=as.factor(c("CCO", "CTH", "EMA", "EMU", "PFO", "SCA","ABC", "ZRA")))
head(mer.files)
```
...
```{r}
# changed accession to V3
merops=merops.raw %>%
  select(V1, V2, V3, Genome) %>%
  rename(Name=`V1`, MERNUM=`V2`, Accession=`V3`) %>%
  separate(Name, into=c("Strain"), sep="\\|")
head(merops)
```

```{r}
# REMOVED COUNT, NOT NEEDED
...
mer.phylo=levels(as.factor(merops.dat$Genome))[c(1, 2, 5, 6, 7, 8, 3, 4)]
```

```{r}
# Open a PDF device
#pdf("merops_upset_plot.pdf", width=10, height=8)

# Generate the plot
UpSetR::upset(fromList(mer.upset.dat), sets=mer.phylo, mb.ratio = c(0.55, 0.45), order.by = "freq", keep.order = TRUE)
...
ggsave(file="plots/MEROPS_UpSet.png", width=11, height=10, dpi=300)
```

```{r}
#MEROPS enrichment
enrich = function(dat, var='DOMAIN', group="Genome", thresh=0.05) {
  out = data.frame()
  vars = unique(dat[[var]])
  len = length(vars)
  ...
  for(i in 1:len) {
    ...
    if (length(Ct) > 0 && length(num) > 0) {
      ...
      if(dim(test)[1] > 0) {
        ...
        test2 = data.frame(Domain = sub, p.value = NA)
      }
    } else {
      print(paste("Skipping MERNUM:", sub, "due to empty counts or numbers"))
      test2 = data.frame(Domain = sub, p.value = NA)
    }
...
```{r}
enrich_debug = function(dat, var='DOMAIN', group="Genome", thresh=0.05) {
  # Check input data
  if (!is.data.frame(dat)) {
    stop("Input data `dat` must be a data frame.")
  }
  if (!(var %in% names(dat)) || !(group %in% names(dat)) || !("Count" %in% names(dat)) || !("n" %in% names(dat))) {
    stop("The data frame `dat` must contain columns: '", var, "', '", group, "', 'Count', and 'n'.")
  }
  ...
  # Check if there are unique values in `var`
  if (len == 0) {
    stop(paste("No unique values found in column:", var))
  }

  print(paste("Number of unique domains:", len))

  for(i in 1:len) {
    sub = vars[i]
    print(paste("Processing:", sub))

    # Subset data for the current domain
    tmp = dat[dat[[var]] == sub, ]
    if (nrow(tmp) == 0) {
      print(paste("No data available for", sub, "- skipping."))
      next
    }

    print("Subset data:")
    print(head(tmp))  # Debugging print
    ...
    # Check if counts and numbers are valid
    if (length(Ct) > 0 && length(num) > 0) {
      # Perform pairwise Fisher's exact test with error handling
      test = tryCatch(
        broom::tidy(pairwise.fisher.test(Ct, num, p.adjust.method="bonferroni")),
        error = function(e) {
          print(paste("Error in pairwise.fisher.test for", sub, ":", e$message))
          return(data.frame())
        }
      )

      # If test results are available, filter by p-value threshold
      if (nrow(test) > 0) {
        print("Test results before filtering:")
        print(test)  # Debugging print

        test2 = test %>%
          dplyr::filter(p.value < thresh) %>%
          dplyr::mutate(Domain = sub)

        print("Significant results after filtering:")
        print(test2)  # Debugging print
      } else {
        print(paste("No significant results for", sub))
        test2 = data.frame(Domain = sub, p.value = NA)
      }
    } else {
      print(paste("Skipping domain:", sub, "due to empty counts or numbers"))
      test2 = data.frame(Domain = sub, p.value = NA)
    }
    ...
    print(out)  # Debugging print after each iteration
  }
...
# Example usage:
# merops.res = enrich_debug(merops.dat, var="MERNUM", group="Genome")
```
...
```{r}
# Fixed the leftjoin
merops.res.stat.meta2=merops.res.stat.meta %>% left_join(mer.key2, by=c("Domain"="MERNUM"))

head(merops.res.stat.meta2)
...
merops.res.stat.meta.metallo=merops.res.stat.meta2 %>%
  ...
  head(merops.res.stat.meta.metallo)
```
...
```{r}
merops.list = unique(subset(merops.res.stat.meta, merops.res.stat.meta$Genome=="CTH")$Domain)
head(merops.list)
```
...
# save the plot
ggsave(file="plots/MEROPS_Down2.png", plot=mer.dwn, width=11, height=10, dpi=300)
```
...
# save the plot
ggsave(file="plots/MEROPS_Up.svg", plot=mer.up, width=11, height=10)
```

```{r}
mer.dwn2=ggplot(subset(Emus.merops.clust, Emus.status=="Down" & Genome=="CTH"), aes(x=Count, y=reorder(Domain, cluster), fill=Fold))+geom_col()+
...
ggsave(file="plots/MEROPS_Down3.png", plot=mer.dwn2, width=11, height=10, dpi=300)
```

```{r}
mer.up2=ggplot(subset(Emus.merops.clust, Emus.status=="Up" & Genome=="CTH"), aes(x=Count, y=reorder(Domain, cluster), fill=Fold))+geom_col()+
  theme+geom_stripes(odd = "#33333333", even = "#00000000")+
...
ggsave(file="plots/MEROPS_Up.png", plot=mer.up2, width=11, height=10, dpi=300)
```
...
ggsave(file="plots/MEROPS_Missing.png", plot=mer.plt3, width=11, height=10, dpi=300)
```

```{r}
Emus.merops.unique=merops.counts %>%
  filter(MERNUM %in% unlist(merops.unique[merops.unique$Genome=="CTH",]$MEROPS))  %>%
  left_join(mer.key2) %>%
  rename("Domain"="MERNUM")
  head(Emus.merops.unique)
...
ggsave(file="plots/MEROPS_Unique.png", plot=mer.plt4, width=11, height=10, dpi=300)
...
ggsave(file="plots/MEROPS_Down_plots.png", plot=mer.down.plt, width=11, height=10, dpi=300)
...
f <- grid.arrange(mer4, mer3, layout_matrix=layout2, widths=c(0.4, 0.6))

# save the plot

ggsave(file="plots/MEROPS_blah.png", plot=f, width=11, height=10, dpi=300)
...
# changed so it uses unique strain, since accession isn't in the Dataframe

cazy.counts=cazy %>%
  group_by(Genome, CAZY) %>%
  summarize(Count=n_distinct(Strain)) %>%
  group_by(Genome) %>%
  mutate(n=sum(Count)) %>%
  group_by(CAZY) %>%
  mutate(n_Genomes=length(unique(Genome)), Genomes=toString(unique(Genome)))
...
ggsave(file="plots/CAZY_UpSet.png", plot=cazy.uplt, width=11, height=10, dpi=300)
...
# print only values above 2 fold in genome CTH
d <- cazy.meta %>% filter(Genome=="CTH" & Fold>2)
print(f, n=100)
library(ggplot2)
# Create a bar plot
ggplot(d, aes(x = reorder(Domain, Fold), y = Fold, fill = Direction)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +  # Flip coordinates for better visibility
  labs(title = "CAZy Regulation in Exophiala crusticola",
       x = "Protein Domains",
       y = "Fold Change vs Genus Mean",
       fill = "Direction") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggsave("exophiala_crusticola_CAZY_plot.png", width = 10, height = 6)
...
cazy.list = unique(subset(cazy.res.stat.meta, cazy.res.stat.meta$Genome=="CTH")$Domain)
head(cazy.list)
...
cazy.plt
# save the plot
ggsave(file="plots/Emus.cazy.clust_plot.png", plot=cazy.plt, width=11, height=10, dpi=300)

```

```{r}

cazy.plt2=ggplot(subset(Emus.cazy.clust, Genome=="CTH"), aes(x=Count, y=reorder(Domain, cluster), fill=Fold))+geom_col()+
  theme+geom_stripes(odd = "#33333333", even = "#00000000")+
...
# save the plot
ggsave(file="plots/Emus.cazy.clust_plot2.png", plot=cazy.plt2, width=11, height=10, dpi=300)
...
# Ssve the plot
ggsave(file="plots/Emus.cazy.clust_plot3.png", plot=cazy.plt3, width=11, height=10, dpi=300)

Emus.cazy.unique=cazy.counts %>%
  filter(CAZY %in% unlist(cazy.unique[cazy.unique$Genome=="CTH",]$cazy))
...
cazy.plt3
# save the plot
ggsave(file="plots/Emus.cazy.clust_plot4.png", plot=cazy.plt3, width=11, height=10, dpi=300)
...
cazy.plt4
# save the plot
ggsave(file="plots/Emus.cazy.clust_plot5.png", plot=cazy.plt4, width=11, height=10, dpi=300)
...
# save the plot
ggsave(file="plots/CAZY_plots_multipanel.png", plot=cazy.1, width=11, height=10, dpi=300)
...
# save the plot
ggsave(file="plots/CAZY_plots_multipanel2.png", plot=cazy.1, width=11, height=10, dpi=300)
```
...
```{r}
# change accession to strain
sigp.pfam=pfam %>%
  left_join(sigp2, by = c("Strain" = "Name")) %>%
  mutate(Prediction=replace_na(Prediction, "Not Secreted")) %>%
  group_by(Pfam, Prediction) %>%
  summarize(Accessions_n=length(unique(Strain)), Accessions=toString(unique(Strain))) %>%
  group_by(Pfam) %>%
...
pfam.counts.acc=pfam %>%
  ...
  head(pfam.counts.acc, 20)


```

```{r}
# here the counts are more than one
pfam.counts.dom=pfam %>%
  ...
  head(pfam.counts.dom)

# print values for ank domain
pfam.counts.dom %>% filter(Pfam=="Ank")
```
### use a single domain to test for fisher's exact test
```{r}
# testing
#pfam.res=enrich(pfam.counts.dom, var="Pfam", group="Genome")
#head(pfam.res)
...
# Define the function
perform_fishers_test <- function(df, pfam_name) {
  # Filter the dataframe for the specified Pfam domain
  filtered_df <- df %>% filter(Pfam == pfam_name)
  
  # Initialize a list to store results
  results_list <- list()
  
  # Loop through each genome
  for (genome in unique(filtered_df$Genome)) {
    # Subset the data for the current genome
    genome_data <- filtered_df %>% filter(Genome == genome)
    
    # Create the contingency table
    contingency_table <- matrix(
      c(
        genome_data$Count[1],   # Count of the Pfam domain
        genome_data$n[1] - genome_data$Count[1],  # Non-Pfam domain counts
        sum(df$Count[df$Genome == genome]),   # Total counts of Pfam domain in genome
        sum(df$n[df$Genome == genome]) - sum(df$Count[df$Genome == genome])  # Non-Pfam counts in genome
      ),
      nrow = 2,
      byrow = TRUE
    )
...
# Example usage:
 pfam_counts_dom <- pfam.counts.dom  # Replace with your actual dataframe
 results <- perform_fishers_test(pfam_counts_dom, "Patatin")
 print(results)
...
pfam.unique=pfam.counts.acc %>%
  ...
  # plot unique Pfam domains for CTH 
  d <- pfam.unique %>% filter(Genome=="CTH")
  print(d, n=100)

# take list of unique Pfam domains for CTH and convert to a dataframe where Pfam domains are rows
pfam.unique2=pfam.unique %>% filter(Genome=="CTH") %>% select(pfam) %>% unnest(pfam)

# join the pfam domains with the counts from the pfam.counts.dom dataframe, extract only value from Count column
pfam.unique3=pfam.unique2 %>% left_join(pfam.counts.dom, by=c("pfam"="Pfam"))
print(pfam.unique3, n=100)

# plot the unique Pfam domains for CTH with counts
library(ggplot2)

# Create a bar plot

ggplot(pfam.unique3, aes(x = reorder(pfam, Count), y = Count, fill = Genome)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +  # Flip coordinates for better visibility
...
ggsave("exophiala_crusticola_unique_pfam_plot.png", width = 10, height = 6)
```
...
ggsave(file="plots/Pfam_UpSet.pdf", plot=pfam.uplt, width=11, height=10, dpi=300)
```
...
# print only values above 2 fold in genome CTH
f <- pfam.meta %>% filter(Genome=="CTH" & Fold>2)
print(f, n=100)
library(ggplot2)
# Create a bar plot
ggplot(f, aes(x = reorder(Domain, Fold), y = Fold, fill = Direction)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip() +  # Flip coordinates for better visibility
  labs(title = "Gene Regulation in Exophiala crusticola",
       x = "Protein Domains",
       y = "Fold Change vs Genus Mean",
       fill = "Direction") +
  theme_minimal() +
...
ggsave("exophiala_crusticola_regulation_plot.png", width = 10, height = 6)
...
pfam.t.files=data.frame(names=str_replace(list.files("Pfam/"), ".domtbl.gz", "")) %>%
  ...
  mutate(source=as.character(1:length(loc)), Genome=as.factor(c("CCO", "CTH", "EMA", "EMU-T", "EMU", "PFO", "SCA", "ABC", "ZRA")))
