...
pfam_plot <- ggplot(data = other_data, aes(y = reorder(Domain, Fold), x = Genome, color = Direction, size = Fold)) +
...
# Create the bar plot
diff_plot <- ggplot(diff_data, aes(x = reorder(Domain, Diff), y = Diff, fill = Diff > 0)) +
  geom_col() +
  theme_minimal() +
...
head(pfam, 20)
```
#### 6: Simplify dataframe so that it only has the domain accessions and the genome it came from
