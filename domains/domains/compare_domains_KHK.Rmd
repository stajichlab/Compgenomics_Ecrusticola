...
cazy_plot <- ggplot(data = other_data, aes(y = reorder(Domain, Fold), x = Genome, color = Direction, size = Fold)) +
  ...
  scale_color_manual(values = c("Up" = "#127852", "Equal" = "#8B8588", "Down" = "#FF0000")) +  # Custom colors for Direction
...
cazy_bar_plot <- ggplot(diff_data, aes(x = reorder(Domain, Diff), y = Diff, fill = Diff > 0)) +
  ...
  scale_fill_manual(values = c("TRUE" = "#127852", "FALSE" = "#FF0000")) +  # Green for increase, red for decrease
  labs(x = "Pfam Domain", y = "ΔCount (E. crusticola - Genus Mean)") +
  coord_flip()

# Show the plot

cazy_comb <- cazy_bar_plot + cazy_plot
cazy_comb
...
# Assign log_p and Domain names directly to the edges of the graph
