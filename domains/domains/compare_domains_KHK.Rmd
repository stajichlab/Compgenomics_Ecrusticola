...
target_genome <- "Exophiala_lecanii-corni"

# Total number of genomes
total_genomes <- unique(cazy.dat$n_Genomes)

# Initialize results dataframe
results <- data.frame(CAZY = character(), p_value = numeric(), odds_ratio = numeric(), stringsAsFactors = FALSE)

# Iterate over each unique CAZy gene family
for (gene in unique(cazy.dat$CAZY)) {
  
  # Number of times the gene appears in the target genome
  a <- sum(cazy.dat$Count[cazy.dat$Genome == target_genome & cazy.dat$CAZY == gene])
  
  # Number of times the gene is absent in the target genome
  b <- sum(cazy.dat$Count[cazy.dat$Genome == target_genome & cazy.dat$CAZY != gene])
  
  # Number of times the gene appears in other genomes
  c <- sum(cazy.dat$Count[cazy.dat$Genome != target_genome & cazy.dat$CAZY == gene])
  
  # Number of times the gene is absent in other genomes
  d <- sum(cazy.dat$Count[cazy.dat$Genome != target_genome & cazy.dat$CAZY != gene])
  ...
  # Store results
  results <- rbind(results, data.frame(CAZY = gene, p_value = test$p.value, odds_ratio = test$estimate))
}
...
results <- results %>% arrange(adjusted_p)
print(results)
...
#### 11: enrichment analysis of pfam domains across genomes
```{R}
...
```
#### 12: Plot enrichment using network analysis
...
E(g)$log_p <- -log10(pfam.res$p.value)
E(g)$domain <- pfam.res$Domain  # Add Domain information to edges

# Create the network plot with ggplot2-style syntax
network <- ggraph(g, layout = "fr") +  # Using "fr" layout for force-directed layout
  ...
  scale_edge_width(range = c(1, 5))  # Adjust edge thickness for better visibility
   ...
   # Adjust edge thickness for better visibility
 # labs(title = "Pfam Domain Enrichment Network", subtitle = "Edge width represents -log10(p-value)", color = "Pfam Domain")

# Save the plot as a PNG file
#ggsave("pfam_domain_enrichment_network.png", plot = f, width = 6, height = 6)
...
```{R} 
  ...
  d <- pfam.unique %>% filter(Genome=="Exophiala_crusticola")
  print(d, n=100)
...
tiger <- ggplot(pfam.unique3, aes(x = reorder(pfam, Count), y = Count, fill = Genome)) +
...
multiplot <- (pfam_bar_plot | pfam_plot) / (cazy_bar_plot | cazy_plot) / (network | tiger)
